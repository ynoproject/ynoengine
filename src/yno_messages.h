...
namespace C2S {
	...
	public:
		ChatPacket(std::string _sys, std::string _msg) : C2SPacket("say"),
			sys(std::move(_sys)), msg(std::move(_msg)) {}
		std::string ToBytes() const override { return Build(sys, msg); }
	protected:
		std::string sys, msg;
	};
	...
	public:
		...
		msg(std::move(_msg)) {}
		std::string ToBytes() const override {
			return Build(map_id, prev_map_id, prev_locations, sys, msg);
		}
	protected:
